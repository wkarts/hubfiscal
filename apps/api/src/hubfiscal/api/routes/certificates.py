...
async def list_certificates(context: AuthContext = Depends(certificate_context), db: AsyncSession = Depends(get_db)):
    if context.tenant_id is None:
        return []
    entities = list((await db.scalars(select(LegalEntity).where(LegalEntity.tenant_id == context.tenant_id))).all())
    enabled_entities = {
    ...
    return [serialize(item) for item in items if item.legal_entity_id is None or item.legal_entity_id in enabled_entities]
...
async def upload_certificate(
    ...
    if legal_entity_id:
        ...
        if "certificates" not in (entity.enabled_resources or []):
            raise HTTPException(status_code=403, detail="Recurso Certificados não habilitado para este CNPJ")
