...
api = tomllib.loads((root / "apps/api/pyproject.toml").read_text(encoding="utf-8"))["project"]["version"]
web = json.loads((root / "apps/web/package.json").read_text(encoding="utf-8"))["version"]
init_content = (root / "apps/api/src/hubfiscal/__init__.py").read_text(encoding="utf-8")
...
for path, value in values.items():
    if value != version:
        raise SystemExit(f"Versão divergente em {path}: {value!r}; esperado {version!r}")
...
print(f"Contrato de versão {version} aprovado em {len(values) + 2} arquivos.")
PY
